...
class CodexUsageMonitor: ObservableObject {
    ...
    func refresh() async {
        ...
        let loaded = try? CodexUsageLoader.load()
        lastRefreshAt = Date()
        // Cache last-known snapshot: a refresh that finds no rollout file (or
        // fails to parse) must NOT wipe the usage we already showed. Only
        // replace when we actually loaded something new.
        guard var fresh = loaded else { return }
        // Carry forward reset timestamps the new snapshot may be missing so the
        // "resets in …" line doesn't vanish between refreshes.
        if let previous = snapshot {
            fresh.windows = fresh.windows.map { window in
                guard window.resetsAt == nil,
                      let cached = previous.windows.first(where: { $0.key == window.key })?.resetsAt
                else { return window }
                var merged = window
                merged.resetsAt = cached
                return merged
            }
        }
        snapshot = fresh
        SoundEffectsService.shared.updateUsageLimit(
            kind: .codex,
            isLimitReached: fresh.rateLimitReachedType != nil ||
                fresh.windows.contains { $0.usedPercentage >= 100 }
        )
    }
}
...
enum CodexUsageLoader {
    ...
    private static func snapshot(from line: String, filePath: String, fallbackTimestamp: Date) -> CodexUsageSnapshot? {
        ...
        return CodexUsageSnapshot(
            ...
            rateLimitReachedType: string(from: rateLimits["rate_limit_reached_type"]),
            windows: windows
        )
    }
