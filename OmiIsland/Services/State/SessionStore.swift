...
actor SessionStore {
    ...
    nonisolated static let logger = Logger(subsystem: "dev.krishna09.omiisland", category: "Session")
    ...
    /// Update conversationInfo for a session.
    ...
    private func processHookEvent(_ event: HookEvent) async {
        ...
        // Codex TUI generates a new UUIDv7 session_id on every launch, and the
        // `startup|resume` matcher fires a SessionStart each time the TUI opens.
        // Creating a session entry here would add a blank session every time the
        // user opens Codex (even before typing a prompt). So skip Codex's first
        // SessionStart and create the session when the real UserPromptSubmit
        // arrives. Claude behavior is unaffected.
        ...
        if event.event == "Stop" {
            ...
            // Timestamp the Stop event separately from lastActivity, which
            // bumps on any hook, so legacy sync consumers can tell true Stop
            // hooks apart from ordinary tool activity.
            //
            ...
            // for the same Claude work turn prevents downstream consumers
            // from firing twice when Claude later
