...
enum AppSettings {
    ...
    /// Apply the user's `anthropicProxyURL` preference to Omi-Island's own
    ...
    /// / ALL_PROXY on Omi-Island's own process means every subprocess
    ...
    /// env vars from `launchctl setenv` (set by the user outside Omi-Island)
